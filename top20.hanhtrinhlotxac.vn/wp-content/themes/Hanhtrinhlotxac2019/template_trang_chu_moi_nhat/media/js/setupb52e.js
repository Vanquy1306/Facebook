...
(function () {
  ...
  const smoothScroll = function (targetEl, duration) {
      const headerElHeight =  document.querySelector('.menu, .regist, .siteHeaderLogo, .regft, .siteHeader__section, .buttoner').clientHeight;
  ...
  const scrollTo = function () {
      const links = document.querySelectorAll('.siteHeader__item, .regist a, .siteHeaderLogo a, .regft a, .siteHeader__section a, .buttoner a');
      links.forEach(each => {
...
const hideNV = document.getElementsByClassName("hide");
const showBtn = document.getElementById("view-all");
const loader = document.getElementById("loader");
const re = document.getElementById("regist-t2");


showBtn.addEventListener("click", function(){
  setInterval(showKH, 2000);
  loader.style.display = "block";
  setInterval(hidenLoader, 2000);
  showBtn.style.display = "none";
  re.classList.add("style");
});



function showKH(){
  for( const hiden of hideNV ){
    hiden.classList.add("block");
  }
}
